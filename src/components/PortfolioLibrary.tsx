...
export const PortfolioLibrary: React.FC<PortfolioLibraryProps> = ({
  ...
  const handleShareCard = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    // Generates client-presentation URL (locks into read-only presentation view, no delete/admin buttons)
    const url = `${window.location.origin}${window.location.pathname}#portfolio=${id}&page=1&mode=client`;
